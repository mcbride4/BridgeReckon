...
def get_snapshot_raspberry(raspberry_id, snapshot_id):
    ...
    cards_snap2 = {}
    for snap in snap1:
        cards_snap1[snap['camera_id']] = snap['cards']
        path1 = snap['path']
        camera_id1 = snap['camera_id']
    for snap in snap2:
        cards_snap2[snap['camera_id']] = snap['cards']
        path2 = snap['path']
        camera_id2 = snap['camera_id']
...
def post_errors():
    ...
    path1 = request.json['path1']
    if path1 != '':
        path1 = path1.split('/')
        path1 = '../static/' + path1[-3] + '/' + path1[-2].replace('2', '1') + '/' + path1[-1]
    path2 = request.json['path2']
    if path2 != '':
        path2 = path2.split('/')
        path2 = '../static/' + path2[-3] + '/' + path2[-2] + '/' + path2[-1]
    path3 = request.json['path3']
    if path3 != '':
        path3 = path3.split('/')
        path3 = '../static/' + path3[-3] + '/' + path3[-2].replace('2', '1') + '/' + path3[-1]
    path4 = request.json['path4']
    if path4 != '':
        path4 = path4.split('/')
        path4 = '../static/' + path4[-3] + '/' + path4[-2] + '/' + path4[-1]
...
@app.route('/errorsolutions/<int:uid>', methods=['GET'])
def get_error_solution(uid):
    error_solution = errorsolutions.find({"uid": str(uid)})[0]
    error_solution.pop('_id', None)
    try:
        return jsonify({'errorsolutions': error_solution})
    except IndexError:
        abort(404)


@app.route('/errorsolutions', methods=['POST'])
def post_error_solution():
    data = request.values.to_dict(flat=False)
    # data = data.replace('\\', '')
    # print(data)
    request.close()
    data['tricks'] = data['errorsolution']
    error_solution = json.dumps(data)
    # print(error_solution)

    errorsolutions.insert_one(data)
    errors.remove({'uid': str(data['uid'][0])})
